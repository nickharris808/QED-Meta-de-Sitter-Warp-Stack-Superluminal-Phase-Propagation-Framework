...
bars = ax.barh(range(len(labels)), values, color=colors, alpha=0.7, edgecolor='black', linewidth=0.5)

# Set labels and title
ax.set_yticks(range(len(labels)))
ax.set_yticklabels(labels, fontsize=14, fontweight='bold')
ax.set_xlabel('Normalized Sensitivity (% change per % parameter change)', fontsize=14, fontweight='bold')
ax.set_title('Parameter Sensitivity Tornado Chart\n(QED-Meta-de Sitter Warp Stack)', fontsize=16, fontweight='bold', pad=30)
...
for i, (bar, value) in enumerate(zip(bars, values)):
    ...
    # Position text label
    if width >= 0:
        x_pos = width + max_abs_value * 0.03
        ha = 'left'
    else:
        x_pos = width - max_abs_value * 0.03
        ha = 'right'
    ...
    ax.text(x_pos, bar.get_y() + bar.get_height()/2, 
            f'{value:.1f}%', ha=ha, va='center', fontsize=12, fontweight='bold')

# Adjust layout
plt.tight_layout(pad=3.0)
plt.subplots_adjust(left=0.3, right=0.85, top=0.9, bottom=0.15)

# Save chart
plt.savefig('tornado_chart_fixed.png', dpi=300, bbox_inches='tight', 
            facecolor='white', edgecolor='none')
print(f"\n✅ Chart saved as: tornado_chart_fixed.png")
plt.close()

print(f"\n🎯 CHART SHOWS CORRECT NORMALIZED VALUES:")
print(f"   - All values shown are % change per % parameter change")
print(f"   - Lens Fraction and Corridor Length are dominant (100% each)")
print(f"   - Chart properly formatted with clear labels and values") 